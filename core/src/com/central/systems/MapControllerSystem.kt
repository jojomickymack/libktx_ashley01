...
class MapControllerSystem : EntitySystem() {
    ...
    private var ais = ImmutableArray(Array<Entity>())
    ...
    override fun addedToEngine(engine: Engine) {
        players = engine.getEntitiesFor(allOf(PhysicsComponent::class, UserControlledComponent::class).get())
        ais = engine.getEntitiesFor(allOf(PhysicsComponent::class, AiControlledComponent::class).get())
    }
    ...
    override fun update(deltaTime: Float) {
        val physicsComponent = mapperFor<PhysicsComponent>()

        players.forEach { player ->
            val physics = player[physicsComponent]
            if (physics != null) {
        ...
        ais.forEach { ai->
            val physics = ai[physicsComponent]
            if (physics != null) {
                val testRect = Rectangle()
